...
$descriptions = @{
    ...
    "github-issues"                  = "Use when creating, managing, commenting on, or closing GitHub issues/PRs - verifying an issue's stated premises against the code before implementing it (does the missing optimization already exist, is the quoted latency reproducible, does the failure reproduce) and correcting the record when a claim is wrong, mandatory Type and Priority labels on all new issues, and mandatory --body-file usage to avoid PowerShell escape mangling."
    "github-labels"                  = "Use when standardizing, applying, or syncing GitHub issue and PR labels across repositories - Type, Priority, and Status taxonomy, requiring Type and Priority on every new issue, and updating labels only on open issues by default."
